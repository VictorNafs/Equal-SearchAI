...
{	
		...
		$reponse = $bdd->query("SELECT * FROM annonce INNER JOIN membres ON annonce.id_membre = membres.id
...
<article class="contenu-article" align="center">
							...
							<?php while($donnees = $reponse->fetch())
							{
								echo $donnees['Titre_annonce'];
									...
									if(!empty($donnees['Photos_annonce']))
	                            	{
	                            		if($reponse)
                        				{
                        				$SelectPhotos_annonce = $bdd->prepare('SELECT Photos_annonce FROM annonce WHERE id_annonce = ?');
                        				$SelectPhotos_annonce->execute(array($_SESSION['id']));
                        				}
                       	...
                       	?>
							...
							<a href="supprimera.php?numDonnees=<?= $donnees['id_annonce'] ?>">Delete </a>
